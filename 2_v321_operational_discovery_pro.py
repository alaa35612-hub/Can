...
class StructuralLiquidityDiscoveryTreeV321:
    ...
    def discovery_pre_rally_candidate(self, candles: List[Candle], idx: int, phase: PhaseMemory) -> Dict[str, Any]:
        """Past-only pre-rally pattern extracted from uploaded discovery data.

        This is deliberately generic: it uses rolling price/base/OI/trades/L-S
        structure, never symbol names, clock times, or future returns.
        """
        if idx < 12:
            return {"active": False, "score": 0.0, "reasons": ["insufficient_history"]}
        c = candles[idx]
        pre24 = candles[max(0, idx - 24):idx]
        pre8 = candles[max(0, idx - 8):idx]
        pre4 = candles[max(0, idx - 4):idx]
        if len(pre8) < 6 or not pre4:
            return {"active": False, "score": 0.0, "reasons": ["insufficient_base_window"]}
        close_med = pval([x.close for x in pre8], 50, c.close)
        base_range = 0.0 if close_med <= 0 else (max(x.close for x in pre8) - min(x.close for x in pre8)) / close_med
        near_base = c.close <= max(x.close for x in pre8) * 1.045 and c.close >= min(x.close for x in pre8) * 0.965
        price24 = 0.0 if not pre24 or pre24[0].close <= 0 else c.close / pre24[0].close - 1
        oi24 = 0.0 if not pre24 or pre24[0].oi <= 0 else c.oi / pre24[0].oi - 1
        oi4 = sum(x.oi_change_pct for x in pre4)
        trades_ratio = ratio_hist(c.trades, [x.trades for x in pre24]) if pre24 else 1.0
        pos_minus_account = c.position_long_pct - c.account_long_pct
        top_position_retention = c.position_lsr >= 1.02 or pos_minus_account >= 3.0 or self.ratio_change(candles, idx, "position_lsr", 4) > -3.0
        crowd_not_chasing = c.account_lsr <= 1.35 and c.global_lsr <= 1.35 and self.ratio_change(candles, idx, "account_lsr", 4) <= 5.0
        reset_or_flush = any(x.oi_change_pct <= -1.5 for x in candles[max(0, idx - 12):idx + 1])
        quiet_or_base = phase.base_detected or (0.004 <= base_range <= 0.14 and near_base)
        constructive_oi = oi24 >= -0.04 and oi4 >= -1.10
        delayed_oi_setup = -0.04 <= oi24 <= 0.07 and -1.10 <= oi4 <= 1.60
        oi_build_setup = oi24 >= 0.035 or oi4 >= 1.25
        trades_ok = trades_ratio >= 0.65 or reset_or_flush or oi_build_setup
        score = 0.0
        reasons: List[str] = []
        def add(cond: bool, pts: float, reason: str) -> None:
            nonlocal score
            if cond:
                score += pts
                reasons.append(reason)
        add(quiet_or_base, 24, "base_or_compression")
        add(constructive_oi, 18, "oi_not_destructive")
        add(oi_build_setup, 16, "oi_build_or_reload")
        add(delayed_oi_setup, 10, "delayed_oi_allowed")
        add(trades_ok, 12, "trades_not_dead")
        add(top_position_retention, 16, "top_position_retention")
        add(crowd_not_chasing, 10, "accounts_global_not_chasing")
        add(reset_or_flush, 8, "recent_reset_or_flush")
        add(-0.08 <= price24 <= 0.18, 8, "not_far_extended")
        active = score >= 62 and quiet_or_base and constructive_oi and top_position_retention and crowd_not_chasing and price24 <= 0.22
        return {
            "active": active,
            "score": round(score, 2),
            "reasons": reasons,
            "base_range": round(base_range, 6),
            "price24": round(price24, 6),
            "oi24": round(oi24, 6),
            "oi4": round(oi4, 6),
            "trades_ratio": round(trades_ratio, 6),
            "position_minus_account": round(pos_minus_account, 6),
            "reset_or_flush": reset_or_flush,
        }
    ...
    def score_all_patterns_v321o(self, F: Dict[str, Any]) -> List[PatternCandidate]:
        P, OI, T, Q, LS, S, PM, OIV, DQ, D = F["price"], F["oi"], F["trades"], F["quote_volume"], F["ls"], F["structure"], F["phase_memory"], F["oi_value"], F["data_quality"], F.get("discovery", {})
        ...
        out = [
            mk("Discovery Pre-Rally Base Compression", [D.get("active", False), S["base_exists"] or D.get("base_range", 1) <= 0.14, not P["breaks_base_low"], not LS["top_position_collapse"]], [D.get("score", 0) >= 70, Q["validation"] == "UNAVAILABLE", OI["state"] in ["FLAT", "GRADUAL_UP", "GRADUAL_DOWN", "DELAYED_RELOAD_FROM_BASE", "PRICE_LED_BASE_MOVE_WITHOUT_OI_EXPANSION"], LS["top_position_retention"]], 91, "Early Bullish Structure", "Primed Structure"),
            mk("Discovery Delayed-OI Base Watch", [D.get("active", False), -0.05 <= D.get("oi24", 0) <= 0.08, D.get("oi4", 0) <= 1.8, LS["top_position_retention"], LS["top_account_not_chasing"], not P["far_from_footprint"]], [D.get("score", 0) >= 68, D.get("trades_ratio", 0) >= 0.65, Q["validation"] == "UNAVAILABLE"], 89, "Early Bullish Structure", "Primed Structure"),
    ...
    def selection_priority_v321o(self, c: PatternCandidate, F: Dict[str, Any]) -> float:
        ...
        if name in {"Discovery Pre-Rally Base Compression", "Discovery Delayed-OI Base Watch"}:
            return 560 + min(120, int(F.get("discovery", {}).get("score", 0))) + c.specificity_rank
    ...
    def assign_bias_v321o(self, selected_pattern: str, readiness: str, F: Dict[str, Any]) -> str:
        P = F["price"]
        if selected_pattern in {"Discovery Pre-Rally Base Compression", "Discovery Delayed-OI Base Watch", "Fresh Long Build-up", "Hidden Buildup / Absorption", "Absorption After Flush", "Short Build Under Stable Price"}:
