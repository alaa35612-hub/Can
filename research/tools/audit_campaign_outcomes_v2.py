...
def matched_controls(
    ...
    for index in range(16, len(rows)):
        ...
        if state not in {"LATENT", "FAILURE", "RESET", "COOLING"}:
            continue
...
def audit_symbol(symbol: str, case_root: Path, profile_root: Path, output_root: Path) -> dict[str, Any]:
    ...
    summary["control_definition"] = (
        "Same-symbol context matches selected only from causal "
        "LATENT/FAILURE/RESET/COOLING cutoffs using information available at each control cutoff."
    )
    summary["method_constraints"] = [
        item.replace(
            "Controls are matched within the same symbol and exclude campaign neighborhoods.",
            "Controls are matched within the same symbol from causal LATENT/FAILURE/RESET/COOLING cutoffs.",
        )
    ...
    report = BASE.render_symbol_summary(summary).replace(
        "outside campaign neighborhoods", "from non-active causal states"
    )
