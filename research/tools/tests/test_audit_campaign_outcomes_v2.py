...
MODULE_PATH = Path(__file__).resolve().parents[1] / "audit_campaign_outcomes_v2.py"
SPEC = importlib.util.spec_from_file_location("audit_campaign_outcomes_v2", MODULE_PATH)
...
class CausalControlTests(unittest.TestCase):
    ...
    def test_controls_exclude_active_states(self):
        rows = make_rows()
        reviewed = [
            {
        ...
        for control in controls:
            self.assertIn(
                control["causal_state"],
                {"LATENT", "FAILURE", "RESET", "COOLING"},
            )
