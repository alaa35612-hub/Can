...
class VacuumIgnitionDetector:
    ...
    def detect_price_led_base_vacuum_ignition(self, df: pd.DataFrame, base_zone: BaseZone, trigger_event: TriggerEvent, acceptance_state: AcceptanceState, timing_state: TimingState, ls_fuel_state: LSFuelState, trades_validation: str, oi_state: str, baseline: Dict[str, Dict[str, Any]]) -> VacuumIgnitionState:
        ...
        if failed and trigger_event.from_base:
            ...
            from_base
            and trigger_event.detected
            and trigger_event.type == "Vacuum / Stop-driven Trigger"
            and trade_quote
            and accepted
            and oi_flat_down
            and oi_not_collapsing
            and top_retained
            and account_non_chasing
            and global_not_long
        ):
...
class ConflictResolver:
    ...
    def resolve(self, df: pd.DataFrame, pre: PreScanState, quality_cap: str, reliability: float, data_flags: List[str], mode: str) -> AnalysisResult:
        ...
        if pre.acceptance_state.state == "Structure Invalidated" and pre.trigger_event.from_base and oi_flat_down_context:
            ...
            if (
                pre.price_state in {"slow downtrend", "violent downtrend"}
                and oi_build_or_retained
                and long_chasing_or_heavy
        ...
        elif pre.vacuum_ignition_state.active and pre.vacuum_ignition_state.state in {"Price-led Base Vacuum Ignition", "Price-led Base Ignition without OI Expansion", "Vacuum / Stop-driven Base Breakout"}:
        ...
        # 9-10: flush behavior.
            ...
            pre.reset_event.detected
            and pre.trigger_event.detected
            and pre.trigger_event.after_reset
            and pre.trigger_event.price_led
            and pre.oi_state in {"flat OI", "OI Deleveraging After Pump"}
            and pre.post_flush_state.price_stabilized
            and pre.post_flush_state.state not in {"Liquidity Exit / Decay", "Deleveraging Continues", "No Reset Context"}
        ):
        ...
        # 11-15: build-up, squeeze and stop-driven. Hidden/short-build branches must precede Fresh Long Build-up.
            ...
            pre.oi_state in {"explosive OI build", "gradual OI build", "Delayed Constructive OI Reload From Base"}
            and pre.price_state in {"sideways/base", "healthy uptrend", "up from base without reset"}
            and not pre.ls_fuel_state.crowd_chasing
            and not pre.ls_fuel_state.account_chasing
            and pre.ls_fuel_state.top_position_retention
            and (
                pre.oi_value_validation in {"Real Position Expansion", "Neutral OI Value Read", "Unavailable"}
                or not CONFIG.get("USE_OI_VALUE_VALIDATION", True)
            )
            and not (pre.trigger_event.detected and pre.trigger_event.far_from_footprint)
        ):
            pattern = "Fresh Long Build-up"
            readiness = "Primed Structure" if not pre.trigger_event.detected else "Confirmed Trigger"
        elif (
            pre.trigger_event.detected
            and (pre.price_state == "explosive up move" or pre.trigger_event.price_expansion_state in {"Shock", "Extreme"})
            and "Short" in pre.ls_fuel_state.fuel_state
            and execution_confirmed
        ):
        ...
        # 16-18: compression branches.
            ...
            pre.ls_fuel_state.fuel_state in {
                "Top Position Long Retention with Crowd/Account Compression",
                "Top Position Retention with Non-Chasing Accounts",
            }
            and pre.compression_state.active
            and pre.price_state not in {"slow downtrend", "violent downtrend"}
        ):
        ...
        # 19-26: risk, bearish, weak/noise/mixed.
        elif pre.evidence_flags["late_oi_crowding"] and not pre.vacuum_ignition_state.active:
            ...
            pre.oi_state in {"explosive OI build", "gradual OI build"}
            and pre.price_state in {"slow downtrend", "violent downtrend"}
            and (
                pre.ls_fuel_state.crowd_against_move
                or pre.ls_fuel_state.account_against_move
                or "Short" in pre.ls_fuel_state.fuel_state
            ...
            pre.base_zone.detected
            and pre.oi_state in {"flat OI", "gradual OI decline"}
            and not pre.compression_state.active
            and not (pre.ls_fuel_state.crowd_against_move or pre.ls_fuel_state.account_against_move)
            and not execution_confirmed
        ):
