...
class BinanceFuturesClient:
    """Public Binance USD-M client with retry/backoff and timestamp alignment."""

    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.session = requests.Session()
        self.session.headers.update({"User-Agent": "structural-liquidity-scanner-v321-dynamic/2.0"})
...
class DynamicBaselineEngine:
    ...
    def enrich(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if d["oi_value_exchange"].notna().any():
            d["oi_value"] = d["oi_value_exchange"]
            d["oi_value_source"] = "exchange_sumOpenInterestValue"
        else:
            d["oi_value"] = d["oi"] * d["close"]
            d["oi_value_source"] = "fallback_oi_times_close"
...
class BaseDetector:
    ...
    def detect_base_zone(self, df: pd.DataFrame, baseline: Dict[str, Dict[str, Any]], ls_context: Optional[LSFuelState] = None) -> BaseZone:
        ...
        for end in range(search_start + min_dur - 1, n):
            for dur in range(min_dur, max_dur + 1):
                ...
                # Compare base width against same-duration rolling widths, not a single candle range.
                rolling_widths = (df["high"].rolling(dur).max() - df["low"].rolling(dur).min()) / df["close"].rolling(dur).median().replace(0, np.nan)
                width_rank = percentile_rank(rolling_widths.dropna(), width)
                range_quiet = width_rank <= rolling_widths.rank(pct=True).median()
                trades_rank = percentile_rank(df["trades"], zone["trades"].median()) if "trades" in df else 0.5
                quote_rank = percentile_rank(df["quote_volume"], zone["quote_volume"].median()) if "quote_volume" in df else 0.5
                trades_quiet = trades_rank <= df["trades"].rank(pct=True).median()
                quote_quiet = quote_rank <= df["quote_volume"].rank(pct=True).median() if "quote_volume" in df else False
                ...
                if not oi_changes.empty:
                    ...
                    oi_stable = negative_state_from_rank(percentile_rank(df["oi_change_pct"].dropna(), worst_oi), robust_z(df["oi_change_pct"].dropna(), worst_oi)) not in {"Shock", "Extreme"}
                ...
                if "top_account_ls_change_pct" in zone and zone["top_account_ls_change_pct"].notna().any():
                    ...
                    account_non_chasing = state_from_rank(percentile_rank(df["top_account_ls_change_pct"].dropna(), acc_chg), robust_z(df["top_account_ls_change_pct"].dropna(), acc_chg)) == "Normal" or acc_chg <= 0
                global_not_long = True
                if "global_ls" in zone and zone["global_ls"].notna().any():
                    last_g = zone["global_ls"].iloc[-1]
                    g_rank = percentile_rank(df["global_ls"].dropna(), last_g)
                ...
                if not prior.empty:
                    ...
                    had_prior_pump = prior_ret > 0 and state_from_rank(percentile_rank(df["price_change"].dropna(), prior["price_change"].max()), robust_z(df["price_change"].dropna(), prior["price_change"].max())) in {"Shock", "Extreme"}
                ...
                bz = BaseZone(
                    ...
                    global_not_excessively_long=bool(global_not_long), notes=[f"candidate evidence={evidence_count}", f"width_rank={width_rank:.2f}"],
                )
                if detected:
                    candidates.append((score, bz))
        if not candidates:
            return BaseZone(notes=["no quiet compression candidate found"])
...
class LSFuelModel:
    ...
    def analyze_ls_fuel(self, df: pd.DataFrame, baseline: Dict[str, Dict[str, Any]], base_zone: BaseZone, trigger_event: TriggerEvent, timing_state: Optional[TimingState] = None) -> LSFuelState:
        ...
        if g_dir == "down" and a_dir == "down" and p_dir in {"flat", "up"}:
...
class TriggerDetector:
    ...
    def detect_trigger_event(self, df: pd.DataFrame, base_zone: BaseZone, reset_event: ResetEvent, baseline: Dict[str, Dict[str, Any]]) -> TriggerEvent:
        ...
        for idx in range(start, len(df)):
            ...
            from_base = base_zone.detected and row["close"] > base_zone.high and idx >= (base_zone.end_idx or 0)
        ...
        return sorted(candidates, key=lambda x: (x[0], x[1].idx or -1), reverse=True)[0][1] if candidates else TriggerEvent(notes=["no trigger from base/reset/dynamic expansion"])
...
class PostTriggerAcceptanceEngine:
    ...
    def evaluate_post_trigger_acceptance(self, df: pd.DataFrame, trigger_event: TriggerEvent, base_zone: BaseZone, reset_event: ResetEvent, baseline: Dict[str, Dict[str, Any]]) -> AcceptanceState:
        ...
        returned_inside = base_zone.detected and last_close <= base_zone.high
        ...
        post_flush_low = df.loc[reset_event.idx:, "low"].min() if reset_event.detected and reset_event.idx is not None else np.nan
        ...
        return AcceptanceState(state, bool(accepted), bool(failed), bool(constructive), bool(returned_inside), bool(held_mid), bool(held_base_high), bool(broke_base_low), bool(broke_post_flush_low), bool(oi_unwind), [f"post_trigger_bars={len(post)}"])
...
class VacuumIgnitionDetector:
    ...
    def detect_price_led_base_vacuum_ignition(self, df: pd.DataFrame, base_zone: BaseZone, trigger_event: TriggerEvent, acceptance_state: AcceptanceState, timing_state: TimingState, ls_fuel_state: LSFuelState, trades_validation: str, oi_state: str, baseline: Dict[str, Dict[str, Any]]) -> VacuumIgnitionState:
        ...
        trade_quote = any(s in trades_validation for s in ["Real Execution Expansion", "Real Capital Activation", "Vacuum Flow", "Large-ticket Execution", "Aggressive Buy Execution"])
        ...
        if failed and trigger_event.from_base:
            ...
            active = True
        elif from_base and trigger_event.detected and trigger_event.type == "Vacuum / Stop-driven Trigger":
            state = "Vacuum / Stop-driven Base Breakout"
            active = True
        elif from_base and price_first and oi_flat_down:
            state = "Price-led Base Ignition without OI Expansion"
            active = True
        else:
            state = "Inactive"
            active = False
...
class StructuralPreScanner:
    ...
    def scan(self, df: pd.DataFrame, baseline: Dict[str, Dict[str, Any]]) -> PreScanState:
        ...
        evidence_flags = {
            ...
            "price_led_reset_ignition_with_oi_reload": timing_state.signal_timing == "Price before OI after Reset" and delayed_reload_state.constructive and acceptance_state.constructive,
            "price_led_base_ignition_without_reset": timing_state.signal_timing == "Price before OI from Base without Reset" and acceptance_state.constructive and not ls_fuel_state.account_chasing,
        ...
        return PreScanState(base_zone, reset_event, post_flush_state, trigger_event, acceptance_state, timing_state, delayed_reload_state, ls_fuel_state, compression_state, vacuum_state, oi_state, price_state, trades_validation, oi_value_validation, evidence_flags, invalidation_flags, risk_overlays)
...
class ConflictResolver:
    ...
    def resolve(self, df: pd.DataFrame, pre: PreScanState, quality_cap: str, reliability: float, data_flags: List[str], mode: str) -> AnalysisResult:
        ...
        # 1-4: invalidation has priority over bullish evidence.
        if pre.acceptance_state.state == "Structure Invalidated":
        ...
        elif pre.vacuum_ignition_state.active and pre.vacuum_ignition_state.state in {"Price-led Base Vacuum Ignition", "Price-led Base Vacuum Candidate", "Price-led Base Ignition without OI Expansion"}:
        ...
        # 11-15: build-up, squeeze and stop-driven.
        ...
        elif pre.oi_state in {"explosive OI build", "gradual OI build"} and "Sell Absorption" in pre.trades_validation:
            pattern, readiness = "Hidden Buildup / Absorption", "Primed Structure"
        elif pre.oi_state in {"explosive OI build", "gradual OI build"} and pre.price_state == "sideways/base" and pre.ls_fuel_state.crowd_against_move:
            pattern, readiness = "Short Build Under Stable Price", "Primed Structure"
        elif pre.trigger_event.detected and pre.price_state == "explosive up move" and "Short" in pre.ls_fuel_state.fuel_state:
        ...
        bias = self.bias_for(pattern, readiness, pre)
        ...
        return AnalysisResult(
            ...
            signal_timing=pre.timing_state.signal_timing, cycle_position=self.cycle_position(pre), price_acceptance=pre.acceptance_state.state,
            oi_read=pre.oi_state, oi_value_validation=pre.oi_value_validation, trades_read=pre.trades_validation,
            quote_volume_validation="Quote Volume confirmed" if any(s in pre.trades_validation for s in ["Quote Volume ↑", "Real Execution", "Real Capital", "Large-ticket"]) else "Quote Volume weak/missing",
    ...
    def bias_for(self, pattern: str, readiness: str, pre: PreScanState) -> str:
        ...
        if pattern in {"Bull Trap Risk", "Failed Base Vacuum Ignition", "Failed Base Ignition"}:
    ...
    def confidence(self, pattern: str, readiness: str, pre: PreScanState, cap: str, reliability: float, data_flags: List[str]) -> str:
        ...
        exec_confirm = any(s in pre.trades_validation for s in ["Real Capital Activation", "Real Execution Expansion", "Large-ticket Execution"])
    ...
    def score(self, pre: PreScanState, pattern: str, readiness: str, confidence: str) -> float:
        ...
        score += 7 if any(s in pre.trades_validation for s in ["Real Capital Activation", "Real Execution Expansion", "Large-ticket Execution"]) else 0
    ...
    def priority_bucket(self, pattern: str, readiness: str, pre: PreScanState) -> int:
        if pattern == "Price-led Base Vacuum Ignition without OI Expansion" and (pre.vacuum_ignition_state.close_to_base or pre.trigger_event.close_to_footprint):
            return 4
        bucket = BUCKET_BY_READINESS.get(readiness, 10)
        if readiness == "Confirmed Trigger" and not (pre.trigger_event.close_to_footprint or pre.vacuum_ignition_state.close_to_base):
            bucket = 3
...
class StructuralLiquidityScanner:
    ...
    def analyze_symbol(self, symbol: str) -> Optional[AnalysisResult]:
        try:
            ...
            if self.config.get("RUN_SANITY_CHECKS"):
                validate_result_enums(result)
                validate_no_rsi_decision_dependency()
                validate_vacuum_exception_logic(result)
            return result
    ...
    def print_results(self, results: Sequence[AnalysisResult]) -> None:
        ...
        print("-" * 198)
        header = f"{'rank':<5}{'symbol':<14}{'pattern':<52}{'bias':<32}{'readiness':<26}{'conf':<13}{'score':<8}{'bucket':<8}{'risk':<30}summary"
        print(header)
        print("-" * 198)
        for rank, r in enumerate(results, start=1):
            print(f"{rank:<5}{r.symbol:<14}{r.dominant_structural_pattern[:50]:<52}{r.structural_bias[:30]:<32}{r.readiness_level[:24]:<26}{r.confidence:<13}{r.score:<8.2f}{r.priority_bucket:<8}{r.invalidation_risk[:28]:<30}{r.final_structural_summary[:90]}")
        print("-" * 198)
